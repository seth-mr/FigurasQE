...
Write-Host "=== Reconstruyendo FigurasQE Authentication Service ===" -ForegroundColor Cyan
Invoke-DockerCompose -Arguments @("up", "-d", "--build", "--force-recreate")

Write-Host ""
Write-Host "=== Esperando health check ===" -ForegroundColor Cyan

$healthUrl = "http://localhost:5041/health"
$maxAttempts = 24
...
for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
    try {
        ...
        if ($response.StatusCode -eq 200) {
            ...
            Write-Host ""
            Write-Host "Swagger: http://localhost:5041/swagger" -ForegroundColor Green
            Write-Host "Contenedor: fqe-auth-service" -ForegroundColor Green
            exit 0
        }
    }
    catch {
...
Invoke-DockerCompose -Arguments @("logs", "--tail", "80", "auth-service")
exit 1
